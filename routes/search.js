...
router.get('', function (req, res, next) {
    ...
    if (req.query.city) {
        var latlon = {
            'url': 'http://api.positionstack.com/v1/forward?query=' + req.query.city + "&access_key=144bc2a9e0e28683692189361eb0c21f",
            'method': 'get'
        }
        request1(latlon, function (error1, response1) {
            if (error1) throw new Error(error1)
            response1.body = JSON.parse(response1.body)
            if(response1.body.data[0] ){

                req.query['latitude'] = response1.body.data[0].latitude
                req.query['longitude'] = response1.body.data[0].longitude
